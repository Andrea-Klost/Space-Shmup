...
public class EnemyShield : MonoBehaviour {
    ...
    /// <summary>
    /// Called by Enemy_4.OnCollisionEnter() and parent's EnemyShield.TakeDamage()
    ...
    public float TakeDamage(float dmg) {
        // Can we pass damage to a protector shield?
        foreach (EnemyShield es in protectors) {
            if (es.isActive) {
                dmg = es.TakeDamage(dmg);
                // If all damage was handled, return 0
                if (dmg == 0) return 0;
            }
        }
        ...
        if (health <= 0) {
            // Deactivate shield
            isActive = false;
            // Return any damage that was not absorbed
            return -health;
        }
