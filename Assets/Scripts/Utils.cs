...
public class Utils : MonoBehaviour {
    /// <summary>
    /// Bezier curve function with any amount of points
    /// </summary>
    /// <param name="u">The amount of interpolation [0..1]</param>
    /// <param name="points">An array of Vector3's to interpolate</param>
    static public Vector3 Bezier(float u, params Vector3[] points) {
        // Set up array
        Vector3[,] vArr = new Vector3[points.Length, points.Length];
        // Fill last row with elements of vList
        int r = points.Length - 1;
        for (int c = 0; c < points.Length; c++) {
            vArr[r, c] = points[c];
        }
        
        // Iterate over remaining rows and interpolate points at each one
        for (r--; r >= 0; r--) {
            for (int c = 0; c <= r; c++) {
        ...
        // When complete vArr[0, 0] holds final interpolated value
        return vArr[0, 0];
    }
